...
class TimeTwoOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    float * init_inputs;
    init_inputs = new float[init_len];
    ...
    // Do the computation.
    OP_REQUIRES(context, dilations_tensor.NumElements() <= 4,
                errors::InvalidArgument("Dilations shape must be (4,)"));

    // Do the computation.
    OP_REQUIRES(context, strides_tensor.NumElements() == 2,
                errors::InvalidArgument("Strides shape must be (2,)"));

    // Do the computation.
    OP_REQUIRES(context, init_tensor.NumElements() <= tensorflow::kint32max,
                errors::InvalidArgument("Too many elements in init inputs tensor"));

    // Do the computation.
    OP_REQUIRES(context, fov_tensor.NumElements() == 2,
                errors::InvalidArgument("FOV (Field of View) shape must be (2,)."));

    TimeTwoFunctor<Device>()(
...
#define REGISTER_CPU()                       \
  REGISTER_KERNEL_BUILDER(                   \
      Name("TimeTwo").Device(DEVICE_CPU),    \
      TimeTwoOp<CPUDevice>);
REGISTER_CPU();
...
#ifdef GOOGLE_CUDA
#define REGISTER_GPU()                                 \
  extern template struct TimeTwoFunctor<GPUDevice>;    \
  REGISTER_KERNEL_BUILDER(                             \
      Name("TimeTwo").Device(DEVICE_GPU),              \
      TimeTwoOp<GPUDevice>);
REGISTER_GPU();
#endif  // GOOGLE_CUDA
